...
\begin{document}
	...
	\begin{figure}%
		...
		\caption{Two histograms of mean normalized distance $ND$ values reported for false alarms, one for FCN-MN (black) and one for SW (white).  Bars in the histogram represent the proportion of detection models ($27$ for FCN-MN and $40$ for SW) whose mean $ND$ (over all false alarm components of all images) falls within the interval of the bin.TODO:redactar}
		\label{fig:AAA_ND}
	\end{figure}
